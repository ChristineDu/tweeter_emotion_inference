...
def feature_extraction():
	...
	while(txt_counts <= twt_sum):
		...
		if(re.match(r"<A=[0-4]>",line) or line == ""):
			...
			cc = wordlist["conjunct"]
			expression = "^("+cc+")/|\\s("+cc+")/"
			fe[3] = fe[3] + len(re.findall(expression, line, re.IGNORECASE))
			fe[4] = fe[4] + len(re.findall(r"/VBD", line, re.IGNORECASE))
			fe[5] = fe[5] + len(re.findall(r"('ll|will|gonna|going/\w+\s+to/)\s+\w+/VB", line, re.IGNORECASE))
			fe[6] = fe[6] + len(re.findall(r",/,\s+", line))
			...
			fe[13] = fe[13] + len(re.findall(r"/RB\s+|/RBR\s+|/RBS\s+", line, re.IGNORECASE))
			...
			fe[17] = fe[17] + len(re.findall(r"/",line))
			# 19th feature calculate average token length
			
			tmp = re.findall(r"^[a-zA-Z]+/|\s[a-zA-Z]+/", line)
			print tmp
...
def readfiles():
	...
	f3.close()
	f4 = open("wordlists/Conjunct",'r')
	conjunct = f4.read().rstrip()
	f4.close()
	f5 = open("wordlists/Slang",'r')
	slang = f5.read().rstrip()
	f5.close()
	wordlist = {
